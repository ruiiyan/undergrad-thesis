...
- Identifying a trade-off or constraint IS analysis (Level 4) only when the writer examines WHY the tension exists by identifying multiple contributing factors or consequences — simply stating that two competing demands exist without explaining their origins or implications does not exceed Level 3
- Analysis (Level 4) requires the writer to examine WHY something happened by breaking down multiple contributing factors — a single cause-and-effect chain ('X caused Y') with no further breakdown does not exceed Level 3
- Identifying a specific mechanism that explains WHY something worked or failed — beyond simply stating that it did — qualifies as analysis (Level 4), even if the section is brief
- Evaluative language such as 'instrumental', 'crucial', 'underscored the importance of' does not constitute evaluation — Level 5 requires the writer to explicitly weigh alternatives, justify decisions, or assess what worked and what did not with specific reasoning
- Listing future intentions qualifies as Apply (Level 3) when connected to specific prior experience — it only remains at Level 2 when the intentions are generic and disconnected from the specific context of the reflection
